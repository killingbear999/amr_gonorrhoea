...
amr_model <- function(t, y, parameters) {
  with(as.list(c(y, parameters)), {
    ...
    # doxy-PEP (D)
    dU_D_H = eta_H * p_d * U_N_H + rho * (1 - w_c) * (T_D_H_0 + T_D_H_t) + rho * (1 - phi) * (T_D_H_c + T_D_H_d2) + nu * (A_D_H_0 + A_D_H_c + A_D_H_t + A_D_H_d2) - (e_d * lambda_H_0 + e_d * f_c * lambda_H_c + e_d * f_t * lambda_H_t + e_d * f_d2 * lambda_H_d2 + xi_n + p_v * eta_H + 1/gamma) * U_D_H + xi_d * U_V_H
    ...
    # doxy-PEP + 4CMenB (V)
    dU_V_H = eta_H * p_v * U_D_H + eta_H * p_d * U_M_H + rho * (1 - w_c) * (T_V_H_0 + T_V_H_t) + rho * (1 - phi) * (T_V_H_c + T_V_H_d2) + nu * (A_V_H_0 + A_V_H_c + A_V_H_t + A_V_H_d2) - (e_vd * lambda_H_0 + e_vd * f_c * lambda_H_c + e_vd * f_t * lambda_H_t + e_vd * f_d2 * lambda_H_d2 + xi_n + xi_d + 1/gamma) * U_V_H
    ...
    # doxy-PEP (D)
    dU_D_L = eta_L * p_d * U_N_L + rho * (1 - w_c) * (T_D_L_0 + T_D_L_t) + rho * (1 - phi) * (T_D_L_c + T_D_L_d2) + nu * (A_D_L_0 + A_D_L_c + A_D_L_t + A_D_L_d2) - (e_d * lambda_L_0 + e_d * f_c * lambda_L_c + e_d * f_t * lambda_L_t + e_d * f_d2 * lambda_L_d2 + xi_n + p_v * eta_L + 1/gamma) * U_D_L + xi_d * U_V_L
    ...
    # doxy-PEP + 4CMenB (V)
    dU_V_L = eta_L * p_v * U_D_L + eta_L * p_d * U_M_L + rho * (1 - w_c) * (T_V_L_0 + T_V_L_t) + rho * (1 - phi) * (T_V_L_c + T_V_L_d2) + nu * (A_V_L_0 + A_V_L_c + A_V_L_t + A_V_L_d2) - (e_vd * lambda_L_0 + e_vd * f_c * lambda_L_c + e_vd * f_t * lambda_L_t + e_vd * f_d2 * lambda_L_d2 + xi_n + xi_d + 1/gamma) * U_V_L
...
run_amr <- function(p_d, p_v) {
  ...
  n_iter <- 1000
  random_integers <- sample(1:5000, size = n_iter, replace = FALSE) # draw random integers without replacement
  ...
  for (i in 1:n_iter) {
    ...
    # high-risk group
    # no intervention (N)
    U_N_H = median(samples_y$y[, 6, 1])
    E_N_H_0 = median(samples_y$y[, 6, 2]) - 5
    A_N_H_0 = median(samples_y$y[, 6, 3])
    S_N_H_0 = median(samples_y$y[, 6, 4])
    T_N_H_0 = median(samples_y$y[, 6, 5])
    E_N_H_c = median(samples_y$y[, 6, 6]) + 3
    A_N_H_c = median(samples_y$y[, 6, 7])
    S_N_H_c = median(samples_y$y[, 6, 8])
    T_N_H_c = median(samples_y$y[, 6, 9])
    E_N_H_t = median(samples_y$y[, 6, 10])
    A_N_H_t = median(samples_y$y[, 6, 11])
    S_N_H_t = median(samples_y$y[, 6, 12])
    T_N_H_t = median(samples_y$y[, 6, 13])
    E_N_H_d2 = median(samples_y$y[, 6, 14]) + 2
    A_N_H_d2 = median(samples_y$y[, 6, 15])
    S_N_H_d2 = median(samples_y$y[, 6, 16])
    T_N_H_d2 = median(samples_y$y[, 6, 17])
    # doxy-PEP (D)
    U_D_H = 0 # 18
    E_D_H_0 = 0 # 19
    A_D_H_0 = 0 # 20
    ...
    # low-risk group
    # no intervention (N)
    U_N_L = median(samples_y$y[, 6, 18])  # 69
    E_N_L_0 = median(samples_y$y[, 6, 19]) - 2
    A_N_L_0 = median(samples_y$y[, 6, 20])
    S_N_L_0 = median(samples_y$y[, 6, 21])
    T_N_L_0 = median(samples_y$y[, 6, 22])
    E_N_L_c = median(samples_y$y[, 6, 23]) + 1
    A_N_L_c = median(samples_y$y[, 6, 24])
    S_N_L_c = median(samples_y$y[, 6, 25])
    T_N_L_c = median(samples_y$y[, 6, 26])
    E_N_L_t = median(samples_y$y[, 6, 27])
    A_N_L_t = median(samples_y$y[, 6, 28])
    S_N_L_t = median(samples_y$y[, 6, 29])
    T_N_L_t = median(samples_y$y[, 6, 30])
    E_N_L_d2 = median(samples_y$y[, 6, 31]) + 1
    A_N_L_d2 = median(samples_y$y[, 6, 32])
    S_N_L_d2 = median(samples_y$y[, 6, 33])
    T_N_L_d2 = median(samples_y$y[, 6, 34])
    # doxy-PEP (D)
    U_D_L = 0
    E_D_L_0 = 0
    A_D_L_0 = 0
    ...
    for (t in 1:(n_years)) {
      ...
      lambda_H_0 = get_lambda(t+6, t_0, c_H, params$beta, params$phi_beta, params$epsilon, C_H_0, N_H, pi_H, C_L_0, N_L, pi_L, isFixed)
      lambda_L_0 = get_lambda(t+6, t_0, c_L, params$beta, params$phi_beta, params$epsilon, C_L_0, N_L, pi_L, C_H_0, N_H, pi_H, isFixed)
      lambda_H_c = get_lambda(t+6, t_0, c_H, params$beta, params$phi_beta, params$epsilon, C_H_c, N_H, pi_H, C_L_c, N_L, pi_L, isFixed)
      lambda_L_c = get_lambda(t+6, t_0, c_L, params$beta, params$phi_beta, params$epsilon, C_L_c, N_L, pi_L, C_H_c, N_H, pi_H, isFixed)
      lambda_H_t = get_lambda(t+6, t_0, c_H, params$beta, params$phi_beta, params$epsilon, C_H_t, N_H, pi_H, C_L_t, N_L, pi_L, isFixed)
      lambda_L_t = get_lambda(t+6, t_0, c_L, params$beta, params$phi_beta, params$epsilon, C_L_t, N_L, pi_L, C_H_t, N_H, pi_H, isFixed)
      lambda_H_d2 = get_lambda(t+6, t_0, c_H, params$beta, params$phi_beta, params$epsilon, C_H_d2, N_H, pi_H, C_L_d2, N_L, pi_L, isFixed)
      lambda_L_d2 = get_lambda(t+6, t_0, c_L, params$beta, params$phi_beta, params$epsilon, C_L_d2, N_L, pi_L, C_H_d2, N_H, pi_H, isFixed)
...
prepare_plot_data <- function(df, scenario, time){
  df %>%
    ...
    select(time, scenario, q25, q50, q75)
}

plot_cases <- function(baseline, pep, vac, both, title, size){
  df <- bind_rows(
    prepare_plot_data(baseline, "Baseline", time),
    prepare_plot_data(pep, "Doxy-PEP", time),
    prepare_plot_data(vac, "4CMenB", time),
    prepare_plot_data(both, "Doxy-PEP + 4CMenB", time)
  )
  
  ggplot(df, aes(x = time, y = q50, color = scenario, fill = scenario)) +
...
final <- (p1 + p2 + p3 + p4 + p5 + plot_layout(ncol = 2, guides = "collect")) &
  theme(
