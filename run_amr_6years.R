...
run_amr <- function(p_d, p_v) {
  ...
  n_iter <- 1000
  random_integers <- sample(1:4000, size = n_iter, replace = FALSE) # draw random integers without replacement
...
############################################################## plot cases ##############################################################
# load("workspace_fixed_0.66_6years.RData")
...
plot_cases <- function(baseline, pep, vac, both, title, size){
  ...
  df$scenario <- factor(
    df$scenario,
    levels = c("No Intervention", "Doxy-PEP", "4CMenB", "Doxy-PEP + 4CMenB")
  )
...
final
save.image("workspace_fixed_0.10_0.RData")
