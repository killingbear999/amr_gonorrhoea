...
uptake_vals <- seq(0, 1, by = 0.1) 
phi_vals    <- seq(0, 1, by = 0.1)
param_grid  <- expand.grid(uptake = uptake_vals, phi = phi_vals)

compute_result <- function(res, strat_name, u, p, idx_2040) {
...
# --------------------------------
# 1. No Intervention (vary phi only)
# --------------------------------
print("No Intervention")
no_int_list <- lapply(phi_vals, function(p) {
  res <- run_amr(p_d = 0, p_v = 0, phi = p)
  
  # replicate across uptake grid (for heatmap completeness)
  data.frame(uptake = uptake_vals) %>%
    rowwise() %>%
    do(compute_result(res, "No Intervention", .$uptake, p, idx_2040))
})

all_results[["No Intervention"]] <- bind_rows(no_int_list)

# --------------------------------
# 2. Doxy-PEP (full grid)
# --------------------------------
print("Doxy-PEP")
doxy_list <- mapply(function(u, p) {
  res <- run_amr(p_d = u, p_v = 0, phi = p)
  compute_result(res, "Doxy-PEP", u, p, idx_2040)
}, param_grid$uptake, param_grid$phi, SIMPLIFY = FALSE)
...
# --------------------------------
print("4CMenB")
vac_list <- mapply(function(u, p) {
  res <- run_amr(p_d = 0, p_v = u, phi = p)
  compute_result(res, "4CMenB", u, p, idx_2040)
}, param_grid$uptake, param_grid$phi, SIMPLIFY = FALSE)
...
both_list <- mapply(function(u, p) {
  res <- run_amr(p_d = u, p_v = u, phi = p)
  compute_result(res, "Doxy-PEP + 4CMenB", u, p, idx_2040)
}, param_grid$uptake, param_grid$phi, SIMPLIFY = FALSE)
...
# save.image("workspace_fixed_heatmap_uptake_failure_final.RData")

load("workspace_fixed_heatmap_uptake_failure_final.RData")
final_df <- final_df %>%
...
# 1. Reshape and Re-label Strains as usual
plot_data <- final_df %>%
  pivot_longer(
...
# Note: Since the tiles might look lighter overall now, black text remains ideal.
...
plot_16_panels <- ggplot(plot_data, aes(x = uptake, y = phi)) +
  geom_tile(aes(fill = Percentage, color = Percentage)) + 
  
  # CRUCIAL FIX: Apply pseudo-log scale to the border colors so they match the tiles
  scale_color_viridis_c(option = "magma", direction = 1, trans = "pseudo_log", guide = "none") + 
  
  # Overlay the 16 letter panel labels (a) to (p)
  ...
  # CRUCIAL FIX: Added trans = "pseudo_log" and explicit pretty breaks.
  # This stops low percentages from immediately collapsing into pure black.
  ...
  labs(
    x = "Doxy-PEP and/or Vaccination Uptake Rate",
    y = "Ceftriaxone Treatment Failure Rate"
  ) +
